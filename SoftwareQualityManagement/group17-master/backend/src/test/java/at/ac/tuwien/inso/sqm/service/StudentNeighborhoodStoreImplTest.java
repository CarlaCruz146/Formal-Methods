...
public class StudentNeighborhoodStoreImplTest {
    ...
    public void itBuildsStudentNeighborhood() throws Exception {
        givenStudentPreferences(new StudentSubjectPreference(1L, 10L, 5.0),
    ...
    private void givenStudentPreferences(
            StudentSubjectPreference... studentSubjectPreferences) {
        Mockito.when(mongoTemplate
                .stream(new Query(), StudentSubjectPreference.class))
    ...
    private void thenCheckSimilarStudents(Long target,
            List<Long> expectedSimilarStudents) throws Exception {
        UserNeighborhood neighborhood =
                neighborhoodStore.getStudentNeighborhood();
        List<Long> actualSimilarStudents =
                asList(toObject(neighborhood.getUserNeighborhood(target)));

        Assert.assertThat(actualSimilarStudents,
                CoreMatchers.equalTo(expectedSimilarStudents));
    }
}