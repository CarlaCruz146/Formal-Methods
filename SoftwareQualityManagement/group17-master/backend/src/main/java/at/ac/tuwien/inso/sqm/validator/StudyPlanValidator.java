...
public class StudyPlanValidator {
    ...
    /**
     * Validates the study plan.
     * @param studyPlan the study plan
     */
    public void validateNewStudyPlan(StudyPlanEntity studyPlan) {

        validateStudyPlan(studyPlan);

        if (studyPlan.getName() == null || studyPlan.getName().isEmpty()) {
            throw new ValidationException(
                    "Name of the study plan cannot be empty");
        }

        if (studyPlan.getEctsDistribution().getMandatory() == null) {
            throw new ValidationException(
                    "Mandatory ects of the study plan cannot be empty");
        }

        if (studyPlan.getEctsDistribution().getMandatory().doubleValue() <= 0) {
            throw new ValidationException(
                    "Mandatory ects of the study plan needs to be greater " +
                            "than 0");
        }

        if (studyPlan.getEctsDistribution().getOptional() == null) {
            throw new ValidationException(
                    "Optional ects of the study plan cannot be empty");
        }

        if (studyPlan.getEctsDistribution().getOptional().doubleValue() <= 0) {
            throw new ValidationException(
                    "Optional ects of the study plan needs to be greater than" +
                            " 0");
        }

        if (studyPlan.getEctsDistribution().getFreeChoice() == null) {
            throw new ValidationException(
                    "Free choice ects of the study plan cannot be empty");
        }

        if (studyPlan.getEctsDistribution().getFreeChoice().doubleValue() <=
                0) {
            throw new ValidationException(
                    "Free choice ects of the study plan needs to be greater " +
                            "than 0");
        }

    }

    private void validateStudyPlan(StudyPlanEntity studyPlan) {
    ...
    public void validateNewSubjectForStudyPlan(
        ...
        if (subjectForStudyPlan.getSubject().getId() == null ||
                subjectForStudyPlan.getSubject().getId().intValue() < 1) {
    ...
    public void validateRemovingSubjectFromStudyPlan(StudyPlanEntity studyPlan,
                                                     Subject subject) {
