...
public class StudentNeighborhoodStoreImpl implements StudentNeighborhoodStore {
    ...
    private static final Logger LOGGER =
            LoggerFactory.getLogger(StudentNeighborhoodStoreImpl.class);

    private static final String STUDENT_NEIGHBORHOOD_CACHE_NAME =
            "student-neighborhood";
    private static final String STUDENT_NEIGHBORHOOD_CACHE_EVICT_CRON =
    ...
    // needed for the cache proxy, to rebuild cache
    ...
    private DataModel buildDataModel() {
        LOGGER.info("Building student subject preference data model");

        Query allQuery = new Query();
        try (CloseableIterator<StudentSubjectPreference> iterator =
                mongoTemplate
                .stream(allQuery, StudentSubjectPreference.class)) {
            return new GenericDataModel(
                    toDataMap(extractRawStudentPreferenceData(iterator), true));
        }
    }

    private FastByIDMap<Collection<Preference>> extractRawStudentPreferenceData(
            CloseableIterator<StudentSubjectPreference> iterator) {
        ...
        iterator.forEachRemaining(it -> {
            ...
            studentPreferences.add(new GenericPreference(it.getStudentId(),
                    it.getSubjectId(), it.getPreferenceValue().floatValue()));
        });
    ...
    private UserSimilarity buildSimilarityIndex(DataModel model) {
        LOGGER.info("Building student subject similarity index");
    ...
    public void rebuildStudentNeighborhood() {
        LOGGER.info("Cleared student neighborhood cache");

        studentNeighborhoodStore
                .getStudentNeighborhood(); // causes the data model to be
        // rebuilt and cached
    }
}
