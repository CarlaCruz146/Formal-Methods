...
public class StudentMyStudyPlansController {
    ...
    private Iterable<StudyPlanRegistration> getStudyPlans() {
        StudentEntity student = studentService
                .findOne(userAccountService.getCurrentLoggedInUser());
    ...
    private String getStudyplanDetailsView(@RequestParam(value = "id") Long id,
        ...
        List<SubjectWithGrade> subjectsForStudyPlanWithGrades =
                studyPlanService.getSubjectsWithGradesForStudyPlan(id);

        // filter by subject type: mandatory, optional or free choice
        List<SubjectWithGrade> mandatory =
                subjectsForStudyPlanWithGrades.stream()
                        .filter(s -> s.getSubjectType()
                                .equals(SubjectType.mandatory))
                        .collect(Collectors.toList());
        List<SubjectWithGrade> optional =
                subjectsForStudyPlanWithGrades.stream()
                        .filter(s -> s.getSubjectType()
                                .equals(SubjectType.optional))
                        .collect(Collectors.toList());
        List<SubjectWithGrade> freeChoice =
                subjectsForStudyPlanWithGrades.stream()
                        .filter(s -> s.getSubjectType()
                                .equals(SubjectType.FREE_CHOICE))
                        .collect(Collectors.toList());
        ...
        double progressMandatory = mandatory.stream()
                .filter(s -> s.getGrade() != null &&
                        s.getGrade().getMark().isPositive()).mapToDouble(
                        s -> s.getSubjectForStudyPlan().getSubject().getEcts()
                                .doubleValue()).sum();

        double progressOptional = optional.stream()
                .filter(s -> s.getGrade() != null &&
                        s.getGrade().getMark().isPositive()).mapToDouble(
                        s -> s.getSubjectForStudyPlan().getSubject().getEcts()
                                .doubleValue()).sum();

        double progressFreeChoice = freeChoice.stream()
                .filter(s -> s.getGrade().getMark().isPositive()).mapToDouble(
                        s -> s.getGrade().getCourse().getSubject().getEcts()
                                .doubleValue()).sum();
