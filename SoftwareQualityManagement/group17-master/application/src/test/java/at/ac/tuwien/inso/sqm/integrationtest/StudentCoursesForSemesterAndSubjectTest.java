...
public class StudentCoursesForSemesterAndSubjectTest {
    ...
    private final Subject subject = new Subject("maths", new BigDecimal(6.0));
    ...
    @Test
    public void getTheOnlyCourseOfCurrentSemesterForSubjectTest()
            throws Exception {

        // given only one course for the ws2016 of the subject "maths"
        Course course =
                courseRepository.save(new Course(subject, ws2016));

        // the user should be redirected directly to the course page
        mockMvc.perform(get("/student/lehrveranstaltungen/semester/subject")
                .with(user("student").roles("STUDENT"))
                .param("subjectId", subject.getId().toString())).andExpect(
                redirectedUrl(
                        "/student/lehrveranstaltungen/" + course.getId()));
    }

    @Test
    public void getManyCoursesOfCurrentSemesterForSubjectTest()
            throws Exception {
        ...
        mockMvc.perform(get("/student/lehrveranstaltungen/semester/subject")
                ...
                .andExpect(view().name("/student/courses-for-subject"))
                .andExpect(model().attribute("coursesForSemesterAndSubject",
