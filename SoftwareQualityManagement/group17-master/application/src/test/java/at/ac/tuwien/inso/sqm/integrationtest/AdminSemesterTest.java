...
@Transactional
@FixedClock
@TestExecutionListeners({FixedClockListener.class,
...
public class AdminSemesterTest {
    ...
    public void itListsAllSemesters() throws Exception {
        ...
        Semester ss2016 = semesterRepository
        ...
        mockMvc.perform(
                ...
                .andExpect(model().attribute("allSemesters",
                        asList(ws2016.toDto(), ss2016.toDto()))).andExpect(
    ...
    public void itListsCurrentSemesterAsWS2020() throws Exception {
        SemesterDto expected =
                new SemesterDto(2020, SemesterTypeEnum.WinterSemester);

        mockMvc.perform(
                get("/admin/semester").with(user("admin").roles("ADMIN")))
                .andExpect(model().attribute("allSemesters", asList(expected)))
                .andExpect(model().attribute("currentSemester", expected));
    }

    @Test
    @FixedClock("2019-05-11T11:00:00Z")
    public void itListsCurrentSemesterAsWS2019WithOlderSemesters()
            throws Exception {
        ...
        SemesterDto expected =
                new SemesterDto(2019, SemesterTypeEnum.SummerSemester);

        mockMvc.perform(
                get("/admin/semester").with(user("admin").roles("ADMIN")))
                .andExpect(model().attribute("allSemesters",
                        asList(expected, ws2016.toDto(), ss2016.toDto())))
                .andExpect(model().attribute("currentSemester", expected));
    }
