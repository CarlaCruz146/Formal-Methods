...
public class AdminSubjectLecturersTest {
    ...
    private final LecturerEntity lecturer1 =
            new LecturerEntity("l1", "lecturer1", "l1@uis.at");
    private final LecturerEntity lecturer2 =
            new LecturerEntity("l2", "lecturer2", "l2@uis.at");
    private final LecturerEntity lecturer3 =
            new LecturerEntity("l3", "lecturer3", "l3@uis.at");
    private final Subject ase = new Subject("ase", new BigDecimal(3.0));
    @Autowired
    ...
    public void addLecturerToSubjectTest() throws Exception {
        // given 3 lecturers, where lecturer1 is a already a lecturer of the
        // subject ase

        // when adding lecturer2 to subject ase
        mockMvc.perform(post("/admin/subjects/" + ase.getId() + "/lecturers")
                .param("lecturerId", lecturer2.getId().toString())
                .with(user("admin").roles("ADMIN")).with(csrf())).andExpect(
    ...
    public void addNonExistingLecturerToSubjectTest() throws Exception {
        ...
        mockMvc.perform(post("/admin/subjects/" + ase.getId() + "/lecturers")
                .param("lecturerId", "1337").with(user("admin").roles("ADMIN"))
                .with(csrf())).andExpect(
                redirectedUrl("/admin/subjects/" + ase.getId().toString()));
    ...
    @Test
    public void removeLecturerFromASubjectForWhichHeAlreadyDoesNotExistTest()
            throws Exception {
    ...
    public void removeLecturerFromNonExistingSubjectIdTest() throws Exception {
        ...
        mockMvc.perform(post("/admin/subjects/" + ase.getId() + "/lecturers/" +
                lecturer2.getId() + "/delete").param("subjectId", "1337")
                .param("lecturerId", lecturer2.getId().toString())
                .with(user("admin").roles("ADMIN")).with(csrf())).andExpect(
                redirectedUrl("/admin/subjects/" + ase.getId().toString()));
    ...
    public void availableLecturersJsonTest() throws Exception {
        ...
        MvcResult result = mockMvc.perform(
                get("/admin/subjects/" + ase.getId() +
                        "/availableLecturers.json")
