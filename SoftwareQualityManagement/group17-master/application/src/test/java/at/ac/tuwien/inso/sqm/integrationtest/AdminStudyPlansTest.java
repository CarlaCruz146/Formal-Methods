...
public class AdminStudyPlansTest extends AbstractStudyPlansTest {
    ...
    public void adminShouldSeeAllStudyPlansTest() throws Exception {
        ...
        mockMvc.perform(
                ...
                .andExpect(model().attribute("studyPlans",
                        asList(studyPlan1, studyPlan2, studyPlan3)));
    }
    ...
    public void adminShouldSeeDetailsOfStudyPlanTest() throws Exception {
        ...
        StudyPlanEntity studyPlan = studyPlanRepository.save(studyPlan1);

        // the admin should see details of the study plan, containing
        // separate lists of mandatory and optional subjects
        mockMvc.perform(get("/admin/studyplans")
                .param("id", studyPlan.getId().toString())
                .with(user("admin").roles("ADMIN")))
                .andExpect(model().attribute("studyPlan", studyPlan)).andExpect(
                model().attribute("mandatory", asList(s1, s2, s3, s6)))
                .andExpect(model().attribute("optional", asList(s5, s4)));
    }
    ...
    public void studyPlanSuccessCreationTest() throws Exception {
        ...
        CreateStudyPlanForm form =
                new CreateStudyPlanForm("Bachelor SE", new BigDecimal(120.0),
                        new BigDecimal(30.0), new BigDecimal(30.0));
        StudyPlanEntity expectedStudyPlan = form.toStudyPlan();

        // the created study plan should be seen
        createStudyPlan(form).andExpect(view().name("admin/studyplan-details"))
                .andExpect(model().attribute("studyPlan", expectedStudyPlan));

        // and integrationtest should be persisted
        List<StudyPlanEntity> allStudyPlans = StreamSupport
                .stream(studyPlanRepository.findAll().spliterator(), false)
    ...
    public void studyPlanFailureCreationNameIsEmptyTest() throws Exception {
        ...
        CreateStudyPlanForm form =
                new CreateStudyPlanForm("", new BigDecimal(120.0),
                        new BigDecimal(30.0), new BigDecimal(30.0));
        StudyPlanEntity wrongStudyPlan = form.toStudyPlan();
    ...
    @Test
    public void studyPlanFailureCreationStudyPlanAlreadyExistsTest()
            throws Exception {
        ...
        CreateStudyPlanForm form = new CreateStudyPlanForm(studyPlan1.getName(),
                new BigDecimal(120.0), new BigDecimal(30.0),
                new BigDecimal(30.0));
        StudyPlanEntity wrongStudyPlan = form.toStudyPlan();
    ...
    public void disableStudyPlanTest() throws Exception {
        StudyPlanEntity studyPlan = studyPlanRepository.save(studyPlan1);
        mockMvc.perform(post("/admin/studyplans/disable/")
                .param("id", studyPlan.getId().toString())
                .with(user("admin").roles(Role.ADMIN.name())).with(csrf()))
                .andExpect((redirectedUrl("/admin/studyplans")))
                .andExpect(it -> {
                    StudyPlanEntity s =
                            studyPlanRepository.findOne(studyPlan.getId());
    ...
    public void addSubjectToStudyPlanSuccessTest() throws Exception {
        ...
        Subject subject = subjectRepository
                .save(new Subject("Operating Systems", new BigDecimal(4.0)));

        //when a mandatory subject is added to the study plan
        mockMvc.perform(post("/admin/studyplans/addSubject")
                .with(user("admin").roles(Role.ADMIN.name()))
                .param("subjectId", subject.getId().toString())
                .param("studyPlanId", studyPlan.getId().toString())
                .param("semester", "1").param("mandatory", "true").with(csrf()))
                .andExpect((redirectedUrl(
                        "/admin/studyplans/?id=" + studyPlan.getId())));
    ...
    public void addNonExistingSubjectToStudyPlanFailureTest() throws Exception {
        ...
        //given study plan and a subject which is already part of
        // integrationtest
        Subject subject = new Subject("Operating Systems", new BigDecimal(4.0));
        subject.setId(1337L);
        //studyPlan1.addSubjects(new SubjectForStudyPlanEntity(subject,
        // studyPlan1, true));
        StudyPlanEntity studyPlan = studyPlanRepository.save(studyPlan1);
        ...
        //the subject should not be part of the mandatory subjects of the
        // study plan
        List<SubjectForStudyPlanEntity> subjectsForStudyPlan = StreamSupport
                .stream(subjectForStudyPlanRepository
                        .findByStudyPlanIdOrderBySemesterRecommendation(
    ...
    /**
     * Tests if a Subjcet can be removed from a study plan and that the
     * browser redirects properly afterwards
     *
     * @throws Exception
     */
    @Test
    public void removeSubjectFromStudyPlanTest() throws Exception {
        ...
        // given subjects in a study plan
        StudyPlanEntity studyPlan = studyPlanRepository.save(studyPlan2);
        SubjectForStudyPlanEntity s1 = subjectForStudyPlanRepository
                .save(new SubjectForStudyPlanEntity(subjects.get(0), studyPlan,
                        true, 1));
        SubjectForStudyPlanEntity s2 = subjectForStudyPlanRepository
                .save(new SubjectForStudyPlanEntity(subjects.get(1), studyPlan,
                        true, 1));

        mockMvc.perform(post("/admin/studyplans/remove/")
                .with(user("admin").roles("ADMIN"))
                .param("studyPlanId", studyPlan.getId().toString())
                .param("subjectId", s2.getSubject().getId().toString())
                .with(csrf())).andExpect((redirectedUrl(
                "/admin/studyplans/?id=" + studyPlan.getId().toString())))
                .andExpect(it -> {
                    List<SubjectForStudyPlanEntity> list =
    ...
    public void registerStudentToStudyPlanTest() throws Exception {
        ...
        StudentEntity student = uisUserRepository
                .save(new StudentEntity("s12345", "student", "student@uis.at"));

        // when registering this student to the study plan
        mockMvc.perform(post("/admin/studyplans/studentAnmelden")
                .with(user("admin").roles("ADMIN"))
                .param("studyPlanId", studyPlan1.getId().toString())
                .param("studentId", student.getId().toString()).with(csrf()))
                .andExpect((redirectedUrl(
                        "/admin/users/" + student.getId().toString())))
                .andExpect(it -> {
                    List<StudyPlanEntity> studyPlans =
                            student.getStudyplans().stream()
    ...
    public void isStudyPlanAddedToStudent() throws Exception {
        ...
        StudentEntity newStudent = stduentRepository
                .save(new StudentEntity("s1123456", "TestUser", "a@c.com"));

        StudyPlanEntity sp = studyplanRepository
                .save(new StudyPlanEntity("TestStudyPlan",
                        new EctsDistributionEntity(BigDecimal.TEN,
        ...
        mockMvc.perform(post("/admin/studyplans/studentAnmelden/")
                ...
                .with(user("admin").roles(Role.ADMIN.name())).with(csrf()))
                .andExpect(redirectedUrl("/admin/users/" + newStudent.getId()))
                .andExpect(it -> {
                    StudentEntity s =
                            stduentRepository.findOne(newStudent.getId());
    ...
    public void availableSubjectsForStudyPlanJsonTest() throws Exception {
        ...
        // given 3 subjects, where all of them contain the string
        // "Engineering", but one is already part of studyPlan2
        studyPlan1.addSubjects(
                new SubjectForStudyPlanEntity(subjects.get(1), studyPlan2,
                        true));
        studyPlanRepository.save(studyPlan2);

        // when searching for "Engineering
        MvcResult result = mockMvc.perform(
                get("/admin/studyplans/json/availableSubjects")
                        .with(user("admin").roles("ADMIN"))
        ...
        assertTrue(result.getResponse().getContentAsString()
                .contains("Model Engineering"));
        assertFalse(result.getResponse().getContentAsString()
                .contains("Software Engineering and Project Management"));
    }
    ...
    public void getRegisterStudentViewTest() throws Exception {
        ...
        mockMvc.perform(get("/admin/studyplans/studentAnmelden")
                .with(user("admin").roles("ADMIN"))
                .param("studentToAddId", student.getId().toString()))
                .andExpect(model().attribute("user", student)).andExpect(
                model().attribute("studyPlans",
                        asList(studyPlan2, studyPlan3)));

    }

    private ResultActions createStudyPlan(CreateStudyPlanForm form)
            throws Exception {
        return mockMvc.perform(post("/admin/studyplans/create")
                .with(user("admin").roles(Role.ADMIN.name()))
