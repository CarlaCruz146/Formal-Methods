...
(*EXERCICIO 4.2 - Formulacão do teorema que garante a correcão da funcão merge*)
Theorem merge_correct: forall (l1 l2:list Z), (Sorted l1 /\ Sorted l2 -> Sorted (merge (l1, l2))) /\ Perm (merge (l1, l2)) (l1 ++ l2).
Admitted.

(*EXERCICIO 4.3(a)*)

Inductive My_Length: nat -> (list nat) -> Prop :=
  | ml_base: My_Length 0 nil
  | ml_step: forall n h t, My_Length n t -> My_Length (S n) (h::t).


Lemma length_correct : forall l:list nat, {x : nat | My_Length x l}.
Proof. 
  intros. induction l.
  simpl. exists 0. apply ml_base.
  simpl. inversion IHl. exists (S x). 
  apply ml_step. assumption.
Qed.

(*EXERCICIO 4.3(b)*)
  
Inductive Sum_pair: nat -> (list (nat*nat)) -> Prop :=
  | sp_base: Sum_pair 0 nil
  | sp_step: forall n h t, Sum_pair n t -> Sum_pair (n + snd h) (h::t).


Lemma sum_pair_correct : forall l:list (nat*nat), {x : nat | Sum_pair x l}.
Proof. 
  intros.
  induction l.
  exists 0. simpl. auto.
  simpl. apply sp_base. elim IHl. intro n. exists (n+snd a). 
  apply sp_step. assumption.
Qed.
...
Recursive Extraction length_correct.
Extraction "length" length_correct.

Recursive Extraction sum_pair_correct.
Extraction "sum_pair" sum_pair_correct.
